    ...
    public class LinkedList
    {
        ...
        public void Push(Node newNode)  // CAS를 이용한 atomic한 Push 연산 (lock-free)
        {
            while (true)
            {
                if (newNode != null)
                {
                    ...
                    //if (Interlocked.CompareExchange(ref headNode.next, newNode, oldHead.next) == oldHead.next)
                    //    break;
                    if ((Interlocked.CompareExchange(ref counter, newCounter, oldCounter) == oldCounter)
                        && (Interlocked.CompareExchange(ref headNode.next, newNode, oldHead.next) == oldHead.next))
                        break;
                }
            }
        }
        ...
        public Node Pop()   // CAS를 이용한 atomic한 Pop 연산 (lock-free)
        {
            ...
            while (true)
            {
                ...
                {
                    //if (Interlocked.CompareExchange(ref counter, newCounter, oldCounter) == oldCounter)
                    //    if (Interlocked.CompareExchange(ref headNode.next, currentNode.next, currentNode) == currentNode)
                    //        break;
                    if ((Interlocked.CompareExchange(ref counter, newCounter, oldCounter) == oldCounter)
                        && (Interlocked.CompareExchange(ref headNode.next, currentNode.next, currentNode) == currentNode))
                        break;
                    //if (Interlocked.CompareExchange(ref headNode.next, currentNode.next, currentNode) == currentNode)
                    //    break;
                }
